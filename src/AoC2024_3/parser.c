...
void tokenize(FILE* file, Program* program) {
  ...
  while(!isAtEnd(&parserContext)) {
    ...
    if (currentInstruction->type == NOOP) {
      ...
      blog(PARSER_LOG, "Instruction: %s %d %d\n",
...
char *get_contents(FILE *file, size_t *out_length) {
  if (!file) {
    blog(PARSER_LOG, "Invalid file pointer.\n");
    return NULL;
  }

  // Seek to the end of the file to determine its size
  if (fseek(file, 0, SEEK_END) != 0) {
    blog(PARSER_LOG, "Failed to seek end of file\n");
    return NULL;
  }

  long length = ftell(file);
  if (length < 0) {
    blog(PARSER_LOG, "Failed to get file length\n");
    return NULL;
  }
  ...
  if (!buffer) {
    blog(PARSER_LOG, "Failed to allocate memory\n");
    return NULL;
  }
  ...
  if (read_size != (size_t)length) {
    blog(PARSER_LOG, "Failed to read entire file\n");
    free(buffer);
    return NULL;
  }
